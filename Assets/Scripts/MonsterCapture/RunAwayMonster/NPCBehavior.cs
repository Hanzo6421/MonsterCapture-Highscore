...
public class NPCBehaviour : MonoBehaviour, ITrappable
{
    public enum State
    {
        Patrol,
    ...
    IEnumerator PatrolState()
    {
        ...
        while (state == State.Patrol)
        {
            ...
            float directonToPlayer = Vector3.Distance(transform.position, player.transform.position);

            if (directonToPlayer <= 10)
            {
                state = State.Running;
            }

            if (directonToPlayer <= 5)
            {
                state = State.Attack;
            }
